...
!function($) {
    $.fn.validation = function(callback,options) {
        ...
        return this.each(function() {
            ...
            validationForm(this, options && options.reset);
        });
    };
    ...
    $.fn.valid=function(object,options){
        ...
        if(myoptions !=null && validationError){
            ...
            myobject.before('<span id="validerrmsg" class="help-block" style="float: left; color: #FF0000;">'+myoptions+'</span>');
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'positive_number', validate: function(value) {return (!/^(?:\d+|\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(value));}, defaultMsg: '请输入正数。'},
            ...
            {name: 'chinese', validate: function(value) {return (!/^[\u4e00-\u9fa5]$/.test(value));}, defaultMsg: '请输入汉字。'},
            ...
            {name: 'version', validate: function(value) {return (!/^\d+(\.\d+){1,2}$/.test(value));}, defaultMsg: '请输入正确的版本格式。'},
            {name: 'separator_semicolon', validate: function(value) {return (value.indexOf('；') > -1);}, defaultMsg: '不能使用中文\"；\"来分隔。'}
        ],
        reqmark:true,
    ...
    var validateField = function(field, valid) { 
        ...
        var maxlength=(el.attr('max-length')?el.attr('max-length'):null);
    ...
    //表单验证方法
    var validationForm = function(obj, reset) {
            ...
            if(fform_style==0){
                $(obj).find(".form-group").each(function(){
                    var el = $(this).find('.form-control'),
                        check_type = el.attr('check-type') && el.attr('check-type').split(' ') || [],
                        th = el.parents('td').prev(), title;
                    reset && th.find('.valid-required').remove();
                    title = th.text();
